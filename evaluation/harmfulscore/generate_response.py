...
arg.add_argument("--dataset", type=str, required=True, help="BeaverTails, StrongReject, HarmBench, SinglePrompt")
args = arg.parse_args()
...
class StrongRejectDataset(Dataset):
    def __init__(self, tokenizer, csv_path, model_name, max_length=512):
        self.tokenizer = tokenizer
        ...
        logger.info(f"Loaded {len(self.prompts)} prompts from StrongReject dataset")
...
class HarmBenchDataset(Dataset):
    def __init__(self, tokenizer, csv_path, model_name, max_length=512):
        ...
        self.prompts = []
        for _, row in self.data.iterrows():
            # The behavior column contains the prompts in HarmBench
            prompt = row['Behavior']
            self.prompts.append(self.chat_template.format(prompt=prompt))
        
        logger.info(f"Loaded {len(self.prompts)} prompts from HarmBench dataset")
...
if __name__ == "__main__":
    ...
    if args.dataset == "SinglePrompt":
        ...
        dataset = StrongRejectDataset(
            tokenizer=tokenizer,
            csv_path="../eval_data/strongreject/strongreject_dataset.csv",
            model_name=args.model
        )
    elif args.dataset == "HarmBench":
        dataset = HarmBenchDataset(
            tokenizer=tokenizer,
            csv_path="../eval_data/harmbench/harmbench_behaviors_text_all.csv",
            model_name=args.model
        )
