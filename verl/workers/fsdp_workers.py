...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def update_actor_for_TB(self, data: DataProto):
        # Support all hardwares
        data = data.to(torch.cuda.current_device())
        ...
        with self.ulysses_sharding_manager:
            ...
            # perform training
            with Timer(name='update_policy', logger=None) as timer:
                metrics = self.actor.update_policy_for_TB(data=data)
            delta_time = timer.last
            global_num_tokens = data.meta_info['global_token_num']
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
            metrics[
    ...
    def compute_log_prob(self, data: DataProto, no_lora=False):
        ...
        with adapter_ctx:
            ...
            with self.ulysses_sharding_manager:
                ...
                entropy, output = self.actor.compute_log_prob(data=data)
                output = DataProto.from_dict(tensors={'old_log_probs': output, 'old_entropy': entropy},
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None, save_only_lora=False):
        ...
        # Skip saving full model checkpoint if save_only_lora is True
