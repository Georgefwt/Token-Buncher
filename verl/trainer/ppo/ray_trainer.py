...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        # Initialize global_steps from environment variable or default to 0
        self.global_steps = int(os.environ.get('GLOBAL_STEPS', '0'))
    ...
    def _save_checkpoint(self):
        ...
        # Pass save_only_lora parameter to save only LoRA parameters if configured
        save_only_lora = self.config.trainer.get('save_only_lora', False)
    ...
    def fit(self):
        ...
        # add tqdm
        if self.config.trainer.training_steps is not None:
            progress_bar = tqdm(total=self.config.trainer.training_steps, initial=self.global_steps, desc="Training Progress")
        else:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for step_index, batch_dict in enumerate(self.train_dataloader):
                metrics = {}
                timing_raw = {}
                ...
                if self.config.trainer.training_steps is not None:
                    is_last_step = self.global_steps >= self.config.trainer.training_steps
                else:
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer('update_actor', timing_raw):
                            if self.config.trainer.defence_mode and step_index % 2 == 0 and step_index >=self.config.trainer.EM_warmup_steps:
                            # if self.config.trainer.defence_mode:
                                print(f"Using trap actor update for step {step_index}")
                                actor_output = self.actor_rollout_wg.update_actor_for_TB(batch)
                            else:
                ...
                #TODO: update reward_extra_infos
                for extra_key, extra_values in batch.non_tensor_batch.items():
                    # only consider the keys that came from the reward_extra_infos
                    if extra_key in reward_extra_infos_dict:
                        arr = extra_values.astype(float)
                        metrics[f"critic/{extra_key}/mean"] = float(arr.mean())
                        metrics[f"critic/{extra_key}/max"] = float(arr.max())
                        metrics[f"critic/{extra_key}/min"] = float(arr.min())
